...
class BidConsumer(AsyncWebsocketConsumer):
	...
	def update_bid(self, new_bid):
		bid = Bid.objects.get(bidder=self.user, lot=self.lot)
		bid.bid = new_bid
		bid.save()
	
	@database_sync_to_async
	def update_lot(self, new_price):
	...
	def get_lot_related_bids(self):
		bids = list(self.lot.bid_set.values("bidder", "bid").order_by("-bid").filter(bid__gte=0))
